...
async def post_upload(request: Request, file: UploadFile = File(...)):
    ...
    logger.info(f"Study set {study_set_id} saved for user {user['email']}")

    return RedirectResponse(url="/dashboard?message=File uploaded and processed.", status_code=303)